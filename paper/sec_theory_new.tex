...
that $t_{\min}\le t\le t_{\max}$ $P$-almost surely, and that the retrieval of every combination
lies in $[0,R]$. Then
\[
E(w_f)\le\Bigl(\frac{t_{\max}}{t_{\min}}\Bigr)^2E(w_r).
\]
\end{enumerate}
\end{proposition}
...
inverse, which is the pattern of Table~\ref{tab:tails} in miniature. Item (ii) bounds the loss by
the squared dynamic range of the transmission, and item (i) at $c=b/\delta$ comes within a
factor of four of the bound. On the physical domain of the EMIT table the transmission spans
about twenty-four decades, and thirteen above the floor $10^{-12}T_{\rm train}$ of
Section~\ref{sec:tq-domain}, so the bound gives no control of the retrieval for weights fitted
on a forward loss; the bound \eqref{eq:Jtau-bound} for $J_\tau$ does not involve the range.
