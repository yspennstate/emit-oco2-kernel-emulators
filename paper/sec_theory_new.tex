...
Let $0<\rho<R\le1$ and consider two equally weighted entries with $a=s=0$ and transmissions
$t_1=1$, $t_2=\delta\in(0,1]$. Two heads predict $t$ and $s$ exactly and the path radiance with
errors $e^A=(0,c\delta)$ and $e^B=(b,0)$, where $0<b,c\le\min\{\rho,R-\rho\}$. For the convex
combination with weight $w$ on $A$, let $F(w)$ be the mean squared radiance error at $\rho$ and
$E(w)$ the mean squared error of the constrained retrieval \eqref{eq:rhobar}. Then
\[
F(w)=\tfrac12\bigl[(1-w)^2b^2+w^2c^2\delta^2\bigr],\qquad
E(w)=\tfrac12\bigl[(1-w)^2b^2+w^2c^2\bigr],
\]
$F$ is minimized at $w_f=b^2/(b^2+c^2\delta^2)$ and $E$ at $w_r=b^2/(b^2+c^2)$, and
\[
\frac{E(w_f)}{E(w_r)}\longrightarrow1+\frac{c^2}{b^2}\quad(\delta\to0),\qquad
F(w_f)\le F(w_r).
\]
For every $\tau\le\delta$ the transmission-weighted objective $J_\tau$ of \eqref{eq:Jtau} equals $E$
and is minimized at $w_r$.
\end{proposition}
...
inverse, which is the pattern of Table~\ref{tab:tails} in miniature.

\paragraph{Numerical weights and threshold convention.}
The driver solves a nonnegative least-squares problem with a heavily weighted row penalizing
$\sum_h w_h-1$ and then normalizes the nonnegative weights to sum to one. This gives feasible
