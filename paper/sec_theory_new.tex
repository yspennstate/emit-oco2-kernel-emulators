...
The experiments below reuse validation rows for early stopping, kernel tuning and
combination; they do not implement this additional independent-selection stage.
Proposition~\ref{prop:selection} therefore specifies a possible protocol, not a
probabilistic certificate for the reported stacks or overlapping partitions.
...
Here $B_\ell$ is a bound on the residual RKHS norm, a hypothesis and not an observed certificate, and the second term zero for
...
$|\bar\rho-\rho|\le\min\{R,(|e_a-\eta|+R|e_t|+R^2t|e_s|)/t\}$. The theorem is an exact-arithmetic statement. In floating point, the added term
$\rho t/q$ can be lost when it is small relative to the spacing of representable numbers
near $a$. Its loss depends on $\rho$, $q$, magnitude and rounding, not on a universal
threshold $t<2^{-52}a$. A rounded radiance may then give zero for $L-a$ even with the
tabulated components. The archived operational diagnostics do not implement an arbitrary-
precision verification of these entries, so a numerical violation count is not a proof
of the exact-arithmetic inequality.
...
\paragraph{The implemented separable surrogate.}
The diagnostic driver \texttt{code/joint\_stack.py} combines $Y_2$ and $Y_3$ with
separate weight vectors. It projects each base albedo onto $[0,S]$ before fitting but
leaves the base fluxes unprojected. With
$\widetilde e_j=\widetilde Y_j-Y_j$ for $j=2,3$, its finite-threshold fitting objective is
...
Crucially, the flux floor is applied \emph{after fitting}, to the combined total flux
when retrieval is scored. It is not inside the fitted loss.
Since $t\ge0$,
\[
...
\end{equation}
The factor-three statement is therefore not the bound for the reported separable fit.
...
$g(1/2)=1>(g(0)+g(1))/2=1/2$. This counterexample does not apply to the raw-error
quadratic actually fitted by the driver.

\paragraph{Numerical weights and threshold convention.}
The historical driver solves a nonnegative least-squares problem with a heavily weighted
row penalizing $\sum_h w_h-1$, then normalizes the nonnegative weights to sum to one.
This yields feasible simplex weights but is not an exact solution of the equality-
constrained quadratic program. Its mean-relative-norm arm uses six iteratively reweighted
least-squares sweeps from the squared-loss solution; no convergence or global-optimality
certificate is recorded. We therefore describe these as approximate fits, without
changing the archived predictions or attributing a new optimizer to them.

The driver expresses a finite threshold as $\tau=uT_{\rm train}$, where $T_{\rm train}>0$
is a training-data flux scale. Table~\ref{tab:v2-stacks} reports $u$, not a flux in physical
units. Its legacy \texttt{inf} option is an unweighted physical-component squared-error
baseline; it is \emph{not} the limit of \eqref{eq:Jtau-sep} as $\tau\to\infty$. Its
stored zero coverage field is a sentinel, not a measured tail probability.
The threshold ladder is a sensitivity analysis. Selecting a deployment threshold
requires an advance coverage rule; minimizing a diagnostic bound over test thresholds
does not establish validation-selected retrieval performance.

The stack candidates in this study are fitted on validation rows already used for other
model decisions. Proposition~\ref{prop:selection} does not certify these fitted weights.
Moreover, the broader study used exploratory test feedback. Fixing a particular fit
before its test evaluation does not make the study an untouched confirmation experiment.
