...
For empirical comparison, a common application passband and conditioning threshold should
be fixed before model evaluation. The utility \path{code/conditioned_reflectance.py}
uses a positive flux scale determined on training data and a common truth-based mask,
which is available in simulation benchmarks. It reports retained coverage, non-finite
inversions and denominator failures separately. Masked errors require sample-level arrays
and cannot be recovered from aggregate summaries. The EMIT comparisons in
Section~\ref{sec:paired-tails} therefore use the all-band statistics.
