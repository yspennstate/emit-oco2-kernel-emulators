...
The statements are as numbered in the text.
...
$F(w)=\E_Pr_w^2$ and $E(w)=\E_P[r_w^2/t^2]$. Pointwise
$r_w^2/t_{\max}^2\le r_w^2/t^2\le r_w^2/t_{\min}^2$, so $F\le t_{\max}^2E$ and
$E\le t_{\min}^{-2}F$ for every $w$, and
\[
E(w_f)\le t_{\min}^{-2}F(w_f)\le t_{\min}^{-2}F(w_r)\le\Bigl(\frac{t_{\max}}{t_{\min}}\Bigr)^2E(w_r).
\]
In the example of (i), $t_{\max}/t_{\min}=1/\delta$, and the ratio at $c=b/\delta$ is at least
$\delta^{-2}/4$.
\end{proof}
