...
per state and band: eight inputs (band wavelength, solar and view zenith, relative azimuth, aerosol optical depth at
550 nm, water-vapour column, ozone column, elevation) and three libRadtran coefficients, the path reflectance rho_path,
the total transmittance T_total and the spherical albedo S. Top-of-atmosphere reflectance is
...
others). The script writes, in the layout of the EMIT export read by emit_campaign.py:
    X.npy   (states, 7)   the seven state inputs, in the order above without the wavelength
    ...
    Y4.npy  (states, 13)  S
and MANIFEST.json with the source digest, the selection, the band order and the digest of every array.

usage: python code/make_libradtran_arrays.py <paired_arrays.npz> <out_dir>
"""
import hashlib
import json
import os
...
t = Y["Y2"] + Y["Y3"]
print(json.dumps({"states": n, "bands": bands, "t_min": float(t.min()), "t_q01": float(np.quantile(t, 0.01)),
                  "s_max": float(Y["Y4"].max()), "sha256": man["sha256"]}))
