# pKANrtm on the release's own protocol, on Kaggle (resumable; plan v4, block E5).
...
BUDGET_H = float(os.environ.get("PK_BUDGET_H", "10.5"))
...
for s in SEEDS:
    lanes.append((f"pkanrtm_s{s}_cats_ood", [py, "bench_run.py", "--corpus", "pkanrtm", "--seed", str(s), "--pkan_cats", "1", "--pkan_split", "ood",
                                             "--families", FAM, "--tag", f"pkanrtm_s{s}_cats_ood"]))
    lanes.append((f"pkanrtm_s{s}_lowfi_cats_ood", [py, "bench_run.py", "--corpus", "pkanrtm", "--seed", str(s), "--pkan_cats", "1", "--pkan_split", "ood",
                                                   "--lowfi", "1", "--families", FAM + ",lowfi_resid", "--tag", f"pkanrtm_s{s}_lowfi_cats_ood"]))
...
for tag, cmd in lanes:
    ...
    print(f"===== {tag} rc={r.returncode} in {dt:.0f}s (elapsed {((time.time()-t_all)/3600):.2f} h)", flush=True)
