"""One Kaggle kernel directory per sixteen-input libRadtran lane (seeds 101-110, addendum 1) from lrt_kaggle_lane.py.
...
T = (HERE / "lrtc_kaggle_lane.py").read_text(encoding="utf-8")
gpu = (sys.argv[1] if len(sys.argv) > 1 else "gpu") == "gpu"
made = []
for seed in range(101, 111):
    tag = f"lrtc_s{seed}_w512"
    slug = f"lrtc-s{seed}-w512-20260923"
    d = HERE / "kernels" / tag
    ...
    meta = {"id": f"yitzchakshmalo/{slug}", "title": slug, "code_file": "lane.py", "language": "python",
            ...
            "enable_internet": True, "dataset_sources": ["yitzchakshmalo/libradtran-s2-13band-cats"],
